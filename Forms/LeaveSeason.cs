		...
		public async Task<TextMessage?> RenderAsync(long userId)
		{
			if (_isConfirmed is bool isConfirmed)
			{
				...
				try
				{
					...
					if (deleteReader.RecordsAffected == 0)
					...
					transaction.Commit();
					return new TextMessage($"Количество удалённых из абонемента (ID {_seasonId}) дат: {deleteReader.RecordsAffected}").NavigateToStart();
				}
				catch
				{
